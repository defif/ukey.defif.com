              ...
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="(address, index) in d_addressList" :key="index" @click="clickAddress(index, $event)" style="position: relative;">
                    <td class="text-left">{{ $store.__s('eth.account') }}</td>
                    <td class="text-left d-flex flex-row justify-start align-center" style="cursor: pointer;">
                      <span class="s-address caption" :class="d_selectedId === index ? 'highlight' : ''">
                        <i class="icon mr-1" style="font-size: 18px;" v-html="d_selectedId === index ? '&#xe804;' : '&#xe9cf;'" @click="copyAddress(index)"></i>
  ...
  methods: {
    ...
    async _showOverlay(index, e) {
      ...
      await this.$usb.cmd('EthereumGetAddress', {
        address_n: [(this.c_protocol | 0x80000000) >>> 0, (this.coinInfo.slip44 | 0x80000000) >>> 0, (0 | 0x80000000) >>> 0, 0, this.$store.__s('trop.account')],
        show_display: true
      })
      this._hideOverlay()
    },
